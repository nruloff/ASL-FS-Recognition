def generate_hand_signing_visualization(pq_path, seq_id, selected_hand):
  ...
  '''
  ex_parquet_df = load_relevant_data_subset(pq_path)
  ...
  for val in palm:
    palm_id_list.append('{}-hand_{}'.format(selected_hand, val))
  palm_df = palm_df[palm_df['body_part_id'].isin(palm_id_list)]
  quick_duplicate = palm_df.copy()
  quick_duplicate = quick_duplicate[quick_duplicate['body_part_id'] == '{}-hand_0'.format(selected_hand)]
  quick_duplicate = quick_duplicate.replace(to_replace='{}-hand_0'.format(selected_hand), value='palm_6')
  for m in np.arange(len(palm)):
    palm_df = palm_df.replace(to_replace='{}-hand_{}'.format(selected_hand, palm[m]), value='palm_{}'.format(m))
  ...
  for finger in fingers:
    ...
    for val in finger_list:
      body_part_id_list.append('{}-hand_{}'.format(selected_hand, val))
  ...
  fig = px.scatter(final_df, x='x', y='y', color='body_part', animation_frame='frame', hover_name='body_part_id', width=600, height=800)
